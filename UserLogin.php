...
<div class="login-container">
        <div class="login-box">
            ...
            <form action="" method="post">
                ...
                <input type="text" id="examkey" name="examkey" required>
            </div>
                <div class="input-group">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $solutionStmt = $conn->prepare("SELECT * FROM questions WHERE exam_key=?");
        $solutionStmt->bind_param("s", $ExamKey);
        $solutionStmt->execute();
        $solutionResult = $solutionStmt->get_result();

        if ($solutionResult->num_rows > 0) {
            $_SESSION['firstname'] = $user['firstname'];
            $_SESSION['lastname'] = $user['lastname'];
            $_SESSION['examkey'] = $ExamKey; 

            echo "Logged in";
            header("Location: userdashbard.php");
            exit();
        } else {
            echo "Invalid Exam Key. Please try again.";
        }
        $solutionStmt->close();
    } else {
